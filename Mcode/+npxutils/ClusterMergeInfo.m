classdef ClusterMergeInfo < handle
    ...
    methods
        ...
        function addToMerge(mi, mergeInd, cluster_ids)
            mi.merges{mergeInd} = npxutils.util.makecol(union(mi.merges{mergeInd}, uint32(cluster_ids)));
            mi.postMergeUpdate();
        end
        ...
        function mergeMerges(mi, mergeInds, addToNewMerge)
            ...
            mi.merges{end+1} = npxutils.util.makecol(unique(newMerge));
        ...
        function mergeClusters(mi, cluster_ids)
            ...
            if isempty(mergeInds)
                mi.merges{end+1} = npxutils.util.makecol(unique(uint32(cluster_ids)));
                mi.postMergeUpdate();
            else
