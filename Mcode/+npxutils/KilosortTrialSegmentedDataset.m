classdef KilosortTrialSegmentedDataset < handle & matlab.mixin.Copyable
    ...
    methods
        function seg = KilosortTrialSegmentedDataset(ks, tsi, trial_ids, varargin)
            ...
            seg.spike_idx = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                spike_idx(spike_mask), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
            if loadCutoff
                cutoff_spike_idx = (1:seg.dataset.nSpikesCutoff)';
                seg.cutoff_spike_idx = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
            ...
            if ~segment_by_trials
                ...
                seg.spike_trial_ind = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                    local_trial_ind_each_spike(spike_mask), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
                if loadCutoff
                    seg.cutoff_spike_trial_ind = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
            ...
            if ~segment_by_clusters
                ...
                seg.spike_cluster_inds = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                    unit_idx_each_spike(spike_mask), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
                if loadCutoff
                    seg.cutoff_spike_cluster_inds = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
            ...
            if loadFeatures
                ...
                seg.amplitudes = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                ks.amplitudes(spike_mask), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
                if loadCutoff
                    seg.cutoff_amplitudes = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                ...
                seg.pc_features = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                    ks.pc_features(spike_mask, :, :), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
                if loadCutoff
                    seg.cutoff_pc_features = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                ...
                seg.template_features = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                    ks.template_features(spike_mask, :), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
                if loadCutoff
                    seg.cutoff_template_features = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
            ...
            seg.spike_templates = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
                ks.spike_templates(spike_mask), 1, [nTrialsSegment, nUnitsSegment], subs(spike_mask, :));
            if loadCutoff
                seg.cutoff_spike_templates = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
            ...
            if loadSync && segment_by_trials
                ...
                seg.sync = npxutils.util.TensorUtils.splitAlongDimensionBySubscripts(...
